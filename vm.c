...
/*
Peeks at the value `distance` from the top of the stack.
- `distance` must be at least 0.
*/
...
#define BINARY_OP(valueType, op)                        \
    ...
    for (;;)
    {
        ...
        {
            ObjString *name = READ_STRING();
            if (tableSet(&vm.globals, name, peek(0))) // tableSet is true if the key is newly inserted
            {
...
InterpretResult interpret(const char *source)
{
    ...
    // Top-level code is in a script object, which is function-like. We wrap it in a closure and call that now.
    ObjClosure *closure = newClosure(function);
    pop();
    push(OBJ_VAL(closure));
    call(closure, 0);

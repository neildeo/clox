...
/*
Mark values in use by the compiler as GC roots
*/
...
static void namedVariable(Token name, bool canAssign)
{
    ...
    if (arg != -1) // -1 means the variable must be global (or undefined)
    {
        getOp = OP_GET_LOCAL;
        setOp = OP_SET_LOCAL;
    }
    else if ((arg = resolveUpvalue(current, &name)) != -1) // The variable is in an enclosing non-global scope
    {
...
/* Parse a dot expression (get or set) */
static void dot(bool canAssign)
{
